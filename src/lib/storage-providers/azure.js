...
class Azure {
    loadFiles() {
        return new Promise((resolve, reject) => {
            ...
            const blobService = storage.createBlobServiceWithTokenCredential(
                `https://${azure.storageAccount}.blob.core.windows.net`,
                tokenCredential
            )
    ...
    updateConnected(state) {
        if (state.router.location.pathname === '/oauth/azure') {
            ...
            storeObject.expires_on = new Date(
                new Date().getTime() + parseInt(query['expires_in'])
            ).getTime()
